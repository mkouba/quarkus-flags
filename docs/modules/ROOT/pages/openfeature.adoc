...
```
<1> The type determines which OpenFeature evaluation method is used. Supported values: `boolean` (default), `string`, `int`, `double`.
...
openFeatureFlags.register("new-feature");

// Register a typed flag
openFeatureFlags.register("welcome-message", FlagType.STRING, "Hello");
