...
{flag:on('my-feature')} <3>
{flag:disabled('my-feature')} <4>
{flag:off('my-feature')} <5>
{flag:string('my-feature')} <6>
{flag:int('my-feature')} <7>
{flag:decimal('my-feature')} <8>
```
<1> Returns `true` if the flag value is `true`, `false` otherwise.
<2> Same as `enabled`.
<3> Same as `enabled`.
<4> Returns `true` if the flag value is `false` (negated boolean).
<5> Same as `disabled`.
<6> Returns the flag value as a string.
<7> Returns the flag value as an integer.
<8> Returns the flag value as a decimal (`BigDecimal`).
...
The value accessor methods (`bool`, `enabled`, `on`, `disabled`, `off`, `string`, `int`, `decimal`) accept an optional default value as the second parameter.
