...
public class CachingDecorator implements FlagProvider {
    ...
    @CacheName("quarkus-flags-cache") <1>
    Cache cache;

    @Override
    public Uni<Collection<Flag>> getFlags() {
        return cache.getAsync(delegate.getId(), k -> { <2>
            return delegate.getFlags().memoize().indefinitely(); <3>  
        });
    }
...
----
<1> Inject the cache named `quarkus-flags-cache`. You can configure the cache based on the cache implementation used. For example, `quarkus.cache.caffeine."quarkus-flags-cache".expire-after-write=10m`. See https://quarkus.io/guides/cache for more information.
<2> The cached value is computed if absent.
<3> The returned `Uni` is memoized so that subsequent subscriptions do not trigger computation again. 
