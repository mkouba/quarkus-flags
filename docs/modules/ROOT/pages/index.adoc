...
public class MyFlags {
    ...
    @RegisterFlag <1>
    static boolean newCheckout = true;
...
----
<1> A boolean flag named `newCheckout` (derived from the field name) with an initial value of `true`. The value is re-read from the field on every computation so changes to the field are reflected.
