...
A provider listed in `before` has lower priority, i.e., flags from the annotated provider take precedence.
A provider listed in `after` has higher priority.
Both `before` and `after` reference provider identifiers.
The ordering is validated at build time; cycles in the ordering result in a deployment error.
