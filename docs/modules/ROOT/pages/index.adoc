...
public class MyFlags {
    ...
    @RegisterFlag(evaluator = TimeSpanFlagEvaluator.ID) <4>
...
<4> A boolean flag using the built-in `TimeSpanFlagEvaluator`. The `@WithMetadata` annotation adds metadata entries to the flag.
