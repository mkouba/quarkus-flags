...
<1> Define a _runtime_ flag for feature `my-feature-bravo` with value `false`.
<2> The metadata of `my-feature-bravo` will contain key "foo" with value "bar".
...
public class MyService {
    ...
    void removeFlag() {
        provider.removeFlag("my-feature-alpha");
    }
}
----
