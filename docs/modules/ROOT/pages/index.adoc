...
Providers that manage flags in-memory (such as the built-in `InMemoryFlagProvider`) return `false` by default because caching would only introduce staleness with no performance benefit.
