...
There are several ways to access feature flags: inject the `io.quarkiverse.flags.Flags` central entry point, inject individual `Flag` instances with `@Feature`, use the static `Flag.get(String)` method, or read `@RegisterFlag`-annotated static fields and methods directly.
...
<7> Reading a `@RegisterFlag`-annotated static field is automatically transformed at build time to compute the current flag value. See <<Declarative flag registration>>.
