...
<2> You can also inject a `Flag` for the given feature. The injected flag is never `null` but subsequent invocations can throw `NoSuchElementException` if no such feature flag exists.
